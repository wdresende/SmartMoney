...
const NewEntryAddressPicker = ({address, onChange}) => {
  const getLocation = (latitude, longitude) => {
    Geocoder.init('AIzaSyAYWDJaz-moSgTKK_bXVFcoD9yqFIG5lyI');
    ...
    Geocoder.from({latitude, longitude})
      ...
      .catch((error) => {
        console.error(
          'NewEntryAddressPicker :: getLocation :: error ao recuperar a localização',
          error,
        );
        Alert.alert(
  ...
  const getPosition = () => {
    Geolocation.getCurrentPosition(
      ...
      (error) => {
        console.error(
          'NewEntryAddressPicker :: getPosition :: error ao recuperar a posição',
          error,
        );
        Alert.alert(
          'Houve um erro ao recuperar sua posição, por favor, tenha certeza que autorizou este aplicativo',
